...
const WalkInServiceCustomer = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      navigate("/dashboard/pos/history");
    } catch (err) {
      toast.error(
        err?.response?.data?.message || "Failed to create walk-in service order"
      );
    } finally {
      setSubmitting(false);
    }
      ...
      <Card className="w-full max-w-3xl mx-auto shadow-2xl rounded-3xl border border-gray-200">
        <CardContent className="p-8 space-y-8">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center mt-8 gap-4">
            ...
            >
              {submitting ? "Submitting..." : "Create Service Order"}
            </Button>
          </div>
        </CardContent>
      </Card>
